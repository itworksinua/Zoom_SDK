            ...
            FirebaseApi.shared.registerUser(mail: emailTextField.text!, pass: passwordTextField.text!) { result, error in
                if let _ = result {
                    guard let userID = Auth.auth().currentUser?.uid else {
                        ...
                        return}
                    guard let deviceId = UserDefaults.standard.string(forKey: "deviceToken") else {return}
                    FirebaseDatabase.shared.uploadUser(userID: userID, name: self.nameTextField.text!, UUID: deviceId)
